...
def test_salary_tasks_never_read_unmasked_text(task, segment):
    """Đường duy nhất đưa cột vào mô hình là dl.text.columns_for."""
    used = set(T.columns_for(task, segmented=segment))
    leaked = used & F.UNMASKED_COLUMNS
    assert not leaked, (
        f"task={task} segment={segment} đọc cột chưa che: {sorted(leaked)}"
    )


@pytest.mark.parametrize("segment", [False, True])
def test_salary_tasks_actually_read_the_masked_mirrors(segment):
    """Bản che phải được nối thật, không chỉ là 'không phải cột gốc'."""
    used = set(T.columns_for(C.TASK_SALARY, segmented=segment))
    expected = "job_title_masked_seg" if segment else "job_title_masked"
    assert expected in used, f"cần {expected} trong {sorted(used)}"


@pytest.mark.parametrize("segment", [False, True])
def test_classification_does_read_the_raw_text(segment):
    """Phép đổi sang bản che chỉ áp cho hai bài lương."""
    used = set(T.columns_for(C.TASK_CATEGORY, segmented=segment))
    assert ("job_title_seg" if segment else "job_title") in used
    assert not (used & {"job_title_masked", "job_title_masked_seg",
...
def test_no_task_reads_a_raw_salary_column():
    for task in C.TASKS:
        for segment in (False, True):
            used = set(T.columns_for(task, segmented=segment))
            assert not (used & set(C.TARGET_LEAK_COLUMNS)), f"{task}/segment={segment}"
