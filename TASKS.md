...
**Currently active:** `T0.3` (commit the dirty docs tree) and `T6.1` (literature survey). `T0.1`, `T0.2`, `T1.1` and `T1.2` are done on this machine; `T1.3`, `T1.4` and `T1.5` can start (embeddings under `artifacts/embeddings/`).
...
| T1.3 | Re-run the classification baseline (`--task category`, with and without `--class-weight`) | todo | — | T1.2 | new rows in `04-results.md` with `eval = dev`; table in docs/06 §5.1 + bao-cao ch.4 updated |
| T1.4 | Re-run the salary baseline (`--task salary`) | todo | — | T1.2 | row in `04-results.md`; docs/06 §5.2, docs/07, bao-cao ch.4 updated |
| T1.5 | Re-run the two linear probes (`scripts/probe_embeddings.py`) as the diagnostic bar | todo | — | T1.2 | two `probe-*` rows in `04-results.md` |
