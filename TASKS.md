...
**Currently active:** `T0.3` (commit the dirty docs tree) and `T6.1` (literature survey). `T0.1`, `T0.2`, `T1.1`, `T1.2` and `T1.3` are done on this machine; `T1.4` and `T1.5` can start (embeddings under `artifacts/embeddings/`); `T2.2` and `T2.3` can start (T1.3 done).
